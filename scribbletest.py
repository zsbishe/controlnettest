...
model.load_state_dict(load_state_dict('./models/control_any4.5_scribble.pth', location='cuda'))
model = model.cuda()
...
# 读取输入图片
input_image_path = 'test_imgs/user_1.png'  # 替换为你的图片路径
input_image = cv2.imread(input_image_path)

# 设置参数
prompt ='a colorful turtle'#'a colorful hot air balloon'#'a turtle'#'a cute cat'#'a red and blue hot air balloon'#'a hot air balloon'#'a red balloon,just a balloon'#a black balloon' #'a black turtle'#"A beautiful and green turtle"  # 替换为你的提示
...
#for i, image in enumerate(generated_images):
    ...
    # 定义保存图像的文件夹路径
output_folder = 'a-scribble-turtle-acolorfulturtle-3'
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# 调用处理函数
generated_images = process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta)
