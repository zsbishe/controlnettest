...
model.load_state_dict(load_state_dict('epoch=999-step=999.ckpt', location='cuda'))
model = model.cuda()
...
# 读取输入图片
input_image_path = 'test_imgs/old.png'  # 替换为你的图片路径
input_image = cv2.imread(input_image_path)

# 设置参数
prompt ='Plaster statue of man'#'Plaster statue of Abraham Lincoln'#'cute and white toy'#"cute and white toy like a cloud"  # 替换为你的提示
...
seed = -1  # 随机种子
eta = 0.0  # DDIM的eta参数
bg_threshold = 0.4  # 根据需要设置背景阈值

# 处理函数
def process(input_image_path, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold):
    with torch.no_grad():
...
#for i, image in enumerate(generated_images):
    ...
    # 定义保存图像的文件夹路径
output_folder ='a-normal-old-Plasterstatueofman-sd-3'#'a-normal-man-PlasterstatueofAbrahamLincoln-sd-1'# 'a-normal-toy-cuteandwhitetoy-sd-3'
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# 调用处理函数
generated_images = process(input_image_path, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold)
